...
module numerical_subroutines
    ...
    subroutine set_matrix(epsilon_vector , a_in , t_in , k_current , hermitian)
        implicit none
        real(kind = dp) , dimension(:) , intent(IN)           :: epsilon_vector
        complex(kind = dp) , dimension(: , :) , intent(INOUT) :: hermitian
        real(kind = dp)                                       :: a_in , t_in , k_current
        integer                                               :: matrix_size
        integer                                               :: i_dir = 1 , j_dir = 1
        complex(kind = dp)                                    :: i_const = (0.0_dp , 1.0_dp)

        matrix_size = size(hermitian , 1)

        hermitian = (0.0_dp , 0.0_dp)

        do i_dir = 1 , matrix_size
            do j_dir = 1 , matrix_size
                if(i_dir == j_dir)then
                    hermitian(i_dir , j_dir) = epsilon_vector(i_dir)
                    print*, i_dir , j_dir , 'first'
            !setting the epsilon vals along the leading diagonal
                elseif((i_dir == j_dir + 1 .or. i_dir == j_dir - 1) .and. &
                &(i_dir /= 1 .and. j_dir /= 2) .and. (i_dir /= 2 .and. j_dir /= 1) .and. &
                & (i_dir /= matrix_size .and. j_dir /= matrix_size - 1) .and. &
                &(i_dir /= matrix_size - 1 .and. j_dir /= matrix_size))then
            !checking valid spaces for -t along main diagonal
                    hermitian(i_dir , j_dir) = -1.0_dp*t_in
                    print*, i_dir , j_dir , 'second'
                 elseif((i_dir == j_dir + 1 .or. i_dir == j_dir - 1) .and. &
                 & (i_dir == matrix_size - 1 .or. j_dir == matrix_size - 1)&
                 & .and. (i_dir /= matrix_size - 1 .or. j_dir /= matrix_size)) then
                     hermitian(i_dir , j_dir) = -1.0_dp*t_in
                     print*, 'success'
                elseif((i_dir == 1 .and. j_dir == matrix_size - 1) .or. (i_dir == matrix_size - 1.and. &
                &j_dir == 1) .or. (i_dir == 2 .and. j_dir == matrix_size) .or.&
                &(j_dir == 2 .and. i_dir == matrix_size))then
            !placing -t adjacent to corners
                    hermitian(i_dir , j_dir) = -1.0_dp*t_in
                    print*, i_dir , j_dir , 'third'
                elseif(i_dir == 1 .and. j_dir == matrix_size)then
            !placing in -ve exponent in top corner
                    hermitian(i_dir , j_dir) = -t_in*(EXP(-1.0_dp*i_const*k_current*a_in))
                elseif(j_dir == 1 .and. i_dir == matrix_size)then
            !placing in +ve exponent in bottom corner
                    hermitian(i_dir , j_dir) = -t_in*(EXP(i_const*k_current*a_in))
                    print*, i_dir , j_dir , 'fourth'
                else
                    print*, 'unable to allocate' , i_dir , j_dir
                end if
            end do
        end do
...
program Nate
    ...
    real(kind = dp) , dimension(4) :: epsilon_vector
    complex(kind = dp) , dimension(4 , 4) :: hermitian

    a_val         = 1.0_dp
    epsilon_a_val = 1.0_dp
    epsilon_b_val = 2.0_dp
    t_val         = 3.0_dp
    N_val         = 1E3

    epsilon_vector = (/epsilon_a_val , epsilon_b_val , 4.0_dp , 5.0_dp/)
...
!     call plotting_subroutine(a_val , epsilon_a_val , epsilon_b_val , t_val , N_val , filename1 , case_type)
    ...
    call set_matrix(epsilon_vector , a_val , t_val , pi , hermitian)
